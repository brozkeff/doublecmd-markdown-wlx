...
test_plugin() {
  ...
  nm -D --defined-only "$ROOT/build/$plugin" | grep -q ' MarkdownWlxVersion$'
...
expected = b'EXT="MD" | EXT="MARKDOWN" | EXT="MDOWN"'
assert buffer.value == expected, buffer.value
license = plugin.MarkdownWlxLicense
...
test_plugin markdown-wlx-x86_64-linux-qt5.wlx
test_plugin markdown-wlx-x86_64-linux-qt6.wlx
test_plugin markdown-wlx-x86_64-linux-gtk2.wlx
test_plugin markdown-wlx-x86_64-linux-gtk3.wlx
